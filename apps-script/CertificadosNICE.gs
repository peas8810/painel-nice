...
const NICE_CERT = Object.freeze({
  ...
  ISSUE_HEADERS:['EVENTO_ID','NOME','EMAIL','CARGA_HORARIA','CODIGO','ASSINATURA_HMAC','STATUS','EMITIDO_EM','ENVIADO_EM','PDF_URL','TENTATIVAS_EMAIL','ULTIMO_ERRO','MOTIVO_REVOGACAO','REVOGADO_EM','TIPO_CERTIFICADO','FUNCAO_EVENTO','TITULO_EVENTO_CUSTOM','CUSTOM_LINK_ID','LIVRO_ATA','REGISTRO_CERTIFICADO','VIA_EMITIDA','INSTITUICAO_EMISSORA','INSTITUICAO_ID','LIVRO_DIGITAL','ANO_LIVRO','REGISTRO_DIGITAL','REGISTRO_ID','HASH_PDF'],
...
function instalarSistemaCertificados(){
  ...
  if(typeof instalarLivroDigitalCertificacoes==='function')instalarLivroDigitalCertificacoes();
...
function niceCertIssueRow_(sh,row,map,event){
  ...
  const instituicao=String(niceCertGetByMap_(sh,row,map,'INSTITUICAO_EMISSORA')||event.INSTITUICAO_EMISSORA||'').trim();
  if(!instituicao)throw new Error('Instituição emissora não informada.');

  let codigo=String(niceCertGetByMap_(sh,row,map,'CODIGO')||'').trim().toUpperCase();
  if(!codigo)codigo=niceCertNewCode_(event.EVENTO_ID);

  let emittedAt=niceCertDate_(niceCertGetByMap_(sh,row,map,'EMITIDO_EM'))||new Date();
  let registry=typeof niceCertBookRegistryFromIssue_==='function'?niceCertBookRegistryFromIssue_(sh,row,map):null;
  if(!registry){
    if(typeof niceCertBookAllocate_!=='function')throw new Error('Módulo Livro Digital de Certificações não instalado.');
    registry=niceCertBookAllocate_({
      instituicao,
      codigo,
      tipo:'PADRAO',
      evento_id:event.EVENTO_ID,
      nome,email,
      evento:event.TITULO_EVENTO,
      carga,
      emitido:emittedAt
    });
    niceCertBookWriteIssue_(sh,row,map,registry);
    niceCertSetByMap_(sh,row,map,'CODIGO',codigo);
    niceCertSetByMap_(sh,row,map,'EMITIDO_EM',emittedAt);
    niceCertSetByMap_(sh,row,map,'INSTITUICAO_EMISSORA',instituicao);
  }

  const canonical=niceCertCanonicalV2_({
    codigo,eventId:event.EVENTO_ID,nome,titulo:event.TITULO_EVENTO,
    data:niceCertIsoDate_(event.DATA_EVENTO),carga,emitido:emittedAt.toISOString(),
    instituicao,livro:registry.livro_display,registro:registry.registro
  });
  const signature=niceCertSign_(canonical);
  const pdf=niceCertGeneratePdf_(event,{
    nome,email,carga,codigo,signature,emitido:emittedAt,
    instituicao_emissora:instituicao,
    livro_digital:registry.livro_display,
    registro_digital:registry.registro
  });
  const hash=typeof niceCertBookSha256Blob_==='function'?niceCertBookSha256Blob_(pdf.blob):'';

  if(typeof niceCertBookFinalize_==='function'){
    niceCertBookFinalize_(registry,{hash_pdf:hash,pdf_url:pdf.url});
  }
  ...
  niceCertSetByMap_(sh,row,map,'HASH_PDF',hash);
  ...
  niceCertLog_('CERTIFICADO_EMITIDO',event.EVENTO_ID,codigo,'Livro Digital: '+registry.livro_display+' · Registro '+registry.registro+' · Enviado para '+email);
}
...
function niceCertGeneratePdf_(event,cert){
  ...
  const body='participou do evento “'+event.TITULO_EVENTO+'”, realizado em '+niceCertBrDate_(event.DATA_EVENTO)
    +(event.LOCAL?' no(a) '+event.LOCAL:'')
    +(event.CAMPUS_UNIDADE?' · '+event.CAMPUS_UNIDADE:'')
    +', com carga horária de '+(cert.carga||'não informada')+'.';
  ...
  niceCertTextSerif_(slide,'NO LIVRO DIGITAL DE CERTIFICAÇÕES',202,268,184,12,7,C.navy,true,'START');
  niceCertClassicRegistryInfo_(slide,{
    line1:String(cert.livro_digital||'Livro não informado'),
    line2:'Registro '+String(cert.registro_digital||'—'),
  ...
  niceCertClassicValidation_(
    ...
    cert.signature,
    cert.emitido,
    String(cert.instituicao_emissora||event.INSTITUICAO_EMISSORA||'').trim()
  );

  pres.saveAndClose();
  Utilities.sleep(2000);

  const sourceFile=DriveApp.getFileById(pres.getId());
  const pdfBlob=sourceFile.getBlob().getAs(MimeType.PDF).setName(cert.codigo+'.pdf');
...
function niceCertClassicFrame_(slide){
  ...
  const outer=slide.insertShape(SlidesApp.ShapeType.RECTANGLE,12,12,696,381);
  outer.getFill().setTransparent();
  outer.getBorder().getLineFill().setSolidFill(C.navy);
  outer.getBorder().setWeight(2.1);

  const inner=slide.insertShape(SlidesApp.ShapeType.RECTANGLE,18,18,684,369);
  inner.getFill().setTransparent();
  inner.getBorder().getLineFill().setSolidFill(C.gold);
  inner.getBorder().setWeight(.8);

  const topBar=slide.insertShape(SlidesApp.ShapeType.RECTANGLE,38,30,84,4);
  topBar.getFill().setSolidFill(C.gold);
...
function niceCertClassicHeader_(slide,title){
  ...
  niceCertTextSerif_(slide,title,78,58,564,43,28,navy,true,'CENTER');
  niceCertTextSerif_(slide,'CERTIFICAÇÃO INSTITUCIONAL COM VALIDAÇÃO DIGITAL',78,101,564,14,8,gray,true,'CENTER');

  const line=slide.insertLine(SlidesApp.LineCategory.STRAIGHT,210,122,510,122);
  line.getLineFill().setSolidFill(gold);
  line.setWeight(1);
}

function niceCertClassicDivider_(slide,x1,y,x2){
  const line=slide.insertLine(SlidesApp.LineCategory.STRAIGHT,x1,y,x2,y);
  line.getLineFill().setSolidFill('#C59A45');
  line.setWeight(.75);
}

function niceCertClassicBookSeal_(slide,x,y,size){
  const navy='#17365D',gold='#C59A45';

  const outer=slide.insertShape(SlidesApp.ShapeType.ELLIPSE,x,y,size,size);
  outer.getFill().setSolidFill(navy);
  outer.getBorder().getLineFill().setSolidFill(gold);
  outer.getBorder().setWeight(2);

  const inner=slide.insertShape(SlidesApp.ShapeType.ELLIPSE,x+5,y+5,size-10,size-10);
  inner.getFill().setTransparent();
  inner.getBorder().getLineFill().setSolidFill(gold);
  inner.getBorder().setWeight(.7);
  ...
  [
    ...
    [left,bottom,cx,bottom-3],
    [cx,top,cx,bottom-3],
    [cx,top,right,top+3],
    [right,top+3,right,bottom],
    [right,bottom,cx,bottom-3]
  ].forEach(a=>{
    const ln=slide.insertLine(SlidesApp.LineCategory.STRAIGHT,a[0],a[1],a[2],a[3]);
    ln.getLineFill().setSolidFill(gold);
    ln.setWeight(1.1);
  });

  for(let i=0;i<3;i++){
    const ly=top+6+i*5;
    let ln=slide.insertLine(SlidesApp.LineCategory.STRAIGHT,left+3,ly,cx-3,ly);
    ln.getLineFill().setSolidFill(gold);
    ln.setWeight(.5);

    ln=slide.insertLine(SlidesApp.LineCategory.STRAIGHT,cx+3,ly,right-3,ly);
    ln.getLineFill().setSolidFill(gold);
    ln.setWeight(.5);
  }
}
...
function niceCertClassicRegistryInfo_(slide,o){
  ...
  const line=slide.insertLine(SlidesApp.LineCategory.STRAIGHT,392,247,392,309);
  line.getLineFill().setSolidFill(gold);
  line.setWeight(.9);

  niceCertTextSerif_(slide,'REGISTRO INSTITUCIONAL',414,248,216,11,8,gold,true,'START');
  niceCertTextSerif_(slide,o.line1||'',414,267,216,11,9,navy,true,'START');
  niceCertTextSerif_(slide,o.line2||'',414,284,216,11,9,navy,true,'START');
  niceCertTextSerif_(slide,o.line3||'1ª Via Emitida',414,301,216,10,9,gray,true,'START');
}
...
function niceCertClassicValidation_(slide,codigo,signature,emitido,instituicao){
  ...
  // Painel começa em Y=321; registro termina em Y=311. Há margem visual real entre os blocos.
  const card=slide.insertShape(SlidesApp.ShapeType.RECTANGLE,40,321,640,64);
  card.getFill().setSolidFill(light);
  card.getBorder().getLineFill().setSolidFill('#D9E1E8');
  card.getBorder().setWeight(.8);

  const qr=UrlFetchApp.fetch(
    'https://quickchart.io/qr?size=360&margin=1&ecLevel=M&text='+encodeURIComponent(verify),
    {muteHttpExceptions:false}
  ).getBlob().setName('qr.png');

  // QR maior, porém inteiramente contido no painel.
  slide.insertImage(qr,48,326,54,54);

  niceCertTextSerif_(slide,'VALIDAÇÃO DIGITAL',114,326,142,10,8,gold,true,'START');
  niceCertTextSerif_(slide,'Autenticidade verificável por QR Code',114,340,238,10,8,navy,true,'START');
  niceCertTextSerif_(slide,'Código: '+codigo,114,354,248,9,7,gray,false,'START');
  niceCertTextSerif_(slide,'protocolo.me/certificados/validar',114,368,244,8,7,blue,true,'START');

  const sep1=slide.insertLine(SlidesApp.LineCategory.STRAIGHT,370,327,370,379);
  sep1.getLineFill().setSolidFill('#D8C79D');
  sep1.setWeight(.8);

  niceCertTextSerif_(slide,'INSTITUIÇÃO EMISSORA',386,330,120,10,7,navy,true,'START');
  niceCertTextSerif_(slide,issuer,386,347,122,23,9,gray,false,'START');

  const sep2=slide.insertLine(SlidesApp.LineCategory.STRAIGHT,518,327,518,379);
  sep2.getLineFill().setSolidFill('#D8C79D');
  sep2.setWeight(.8);
  ...
  niceCertTextSerif_(
    slide,
    Utilities.formatDate(emitido,Session.getScriptTimeZone()||'America/Sao_Paulo','dd/MM/yyyy HH:mm'),
    526,370,132,9,7,navy,true,'END'
  );
}
...
function niceCertPremiumFrame_(slide){
  return niceCertClassicFrame_(slide);
}

function niceCertPremiumHeader_(slide,title,subtitle){
  return niceCertClassicHeader_(slide,title);
}

function niceCertPremiumValidation_(slide,codigo,signature,emitido,instituicao){
  return niceCertClassicValidation_(slide,codigo,signature,emitido,instituicao);
}
...
function niceCertPublicVerify_(code){
  ...
  if(/^CERT-NICE-CUS-[A-Z0-9]{10}$/.test(wanted)&&typeof niceCertCustomVerify_==='function')return niceCertCustomVerify_(wanted);
  ...
  if(v.length<2)return{ok:true,valid:false,error:'Certificado não encontrado.'};
  ...
  for(let i=1;i<v.length;i++){
    ...
    if(tipo==='CUSTOMIZADO'||/^CERT-NICE-CUS-/.test(wanted)){
      ...
      pdf=niceCertCustomGeneratePdf_(link,{
        ...
        instituicao_emissora:String(v[i][m.INSTITUICAO_EMISSORA]||link.instituicao_emissora||'').trim()
      });
    }else{
      const event=niceCertFindEvent_(Number(v[i][m.EVENTO_ID]));
      if(!event)return{ok:false,error:'Evento associado não localizado.'};
      pdf=niceCertGeneratePdf_(event,{nome,email,carga,codigo:wanted,signature,emitido:emitted,instituicao_emissora:String(v[i][m.INSTITUICAO_EMISSORA]||event.INSTITUICAO_EMISSORA||'').trim(),livro_digital:String(v[i][m.LIVRO_DIGITAL]||'').trim(),registro_digital:String(v[i][m.REGISTRO_DIGITAL]||'').trim()});
    }
    niceCertSetByMap_(sh,i+1,m,'PDF_URL',pdf.url);
    const hash=typeof niceCertBookSha256Blob_==='function'?niceCertBookSha256Blob_(pdf.blob):'';
    niceCertSetByMap_(sh,i+1,m,'HASH_PDF',hash);
    const reg=typeof niceCertBookFindByCode_==='function'?niceCertBookFindByCode_(wanted):null;
    if(reg&&typeof niceCertBookFinalize_==='function')niceCertBookFinalize_(reg,{hash_pdf:hash,pdf_url:pdf.url});
...
function niceCertRevokeByCode_(code,reason){
  ...
  for(let i=1;i<v.length;i++){
    ...
    niceCertSetByMap_(sh,i+1,m,'REVOGADO_EM',revokedAt);
    if(typeof niceCertBookMarkRevokedByCode_==='function')niceCertBookMarkRevokedByCode_(wanted,motivo,revokedAt);
...
function niceCertCanonicalV2_(o){return[o.codigo,'BOOK_V2',o.eventId,o.nome,o.titulo,o.data,o.carga,o.instituicao,o.livro,o.registro,o.emitido].map(v=>String(v==null?'':v).trim()).join('|')}
