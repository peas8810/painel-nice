(()=>{
  ...
  async function loadEvent(id){
    try{
      const r=await fetch(`/certificados/data/events/${encodeURIComponent(id)}.json?ts=${Date.now()}`,{cache:'no-store'});
    ...
    }catch(err){content.classList.remove('loading');content.innerHTML=`<h2>Evento não localizado</h2><p class="desc">${esc(err.message||'Não foi possível carregar este evento.')}</p><div class="actions"><a class="btn" href="/certificados/">Voltar aos certificados</a></div>`}
  }
